...
const OrderDetails = () => {
    ...
    const fetchOrderDetails = async (isRetry = false) => {
        try {
            ...
            console.log(`📦 Fetching order details for ID: ${id}${isRetry ? ' (retry)' : ''}`);
            ...
            const isTimeoutOrNetworkError =
                ...
                (!error.response && !isRetry);

            if (isTimeoutOrNetworkError && !isRetry) {
                console.log('🔁 Retrying once — server may be waking up...');
                setRetryAttempt(1);
                await sleep(1500);
            ...
            if (isTimeoutOrNetworkError) {
                setError('The server is taking longer than expected to respond. This can happen if it just woke up from being idle — please try again.');
                ...
                setError(error.response?.data?.message || error.message || 'Failed to load order details. Please try again.');
            }
        } finally {
            setLoading(false);
        ...
        return (
            <div style={{
                ...
                <p style={{ color: colors.textSecondary, fontSize: '1.1rem' }}>
                    {retryAttempt > 0
                        ? 'Still working on it — the server may be waking up...'
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <h2 style={{ color: colors.textPrimary, fontSize: '1.8rem', marginBottom: '10px' }}>
                        {error?.toLowerCase().includes('timeout') || error?.toLowerCase().includes('taking longer') ? 'Request Timeout' : 'Order Not Found'}
                    </h2>
                    ...
                    <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
                        <button 
                            onClick={() => { setRetryAttempt(0); fetchOrderDetails(); }}
                            style={{
                                background: colors.primary,
                                color: colors.white,
                                border: 'none',
                                ...
                                <div style={{ color: colors.textSecondary }}>
                                    ...
                                    {order.status === 'cancelled' ? (
                                        <div style={{
                                            ...
                                            <p style={{ color: colors.cancelled, marginBottom: '5px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <XCircle size={16} />
                                                Payment Cancelled
                                            </p>
                                        </div>
                                    ) : order.isPaid === true || order.paymentResult?.status === 'completed' || order.paymentResult?.id ? (
                                        <div style={{
                                            ...
                                            <p style={{ color: colors.success, marginBottom: '5px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <CheckCircle size={16} />
                                                ✓ Payment Successful
                                            </p>
                                            {order.paidAt && (
                                            ...
                                            {order.paymentResult?.id && (
                                                <p style={{ fontSize: '0.8rem', color: colors.textLight, marginTop: '5px' }}>
                                        ...
                                        <div style={{
                                            ...
                                            <p style={{ color: colors.warning, marginBottom: '5px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <Clock size={16} />
                                                ⏳ Payment Pending
                                            </p>
                                        </div>
